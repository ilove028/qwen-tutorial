...
async function main() {
  ...
  const UserInfoSchema = z.object({
    ...
    company: z.string().describe('客户公司'),
    id: z.string().describe('客户ID'),
  })
  const schema = z.object({
    customerInfo: UserInfoSchema.describe('客户信息'),
    version: z.string().describe('软件版本'),
    errorCode: z.string().describe('错误代码'),
    problemDesc: z.string().describe('问题描述'),
    os: z.string().describe('操作系统'),
    otherDetails: z.array(z.string()).nullable().describe('其他相关细节')
  })
  const openai = new OpenAI(
    ...
    const completion = await openai.chat.completions.create({
        ...
        messages: [
            ...
            { role: "user", content: `仔细阅读下面邮件，提取出Schema为${JSON.stringify(z.toJSONSchema(schema))}的json格式信息
